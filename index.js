...
function processSum(numberList, callback) {
  var sum = numberList.reduce((a, b)=>{return a+b}, 0);
 return callback(sum);
}
...
function firstNamesAllCaps(runners) {
  let names = [];
   runners.map(
    function (name) {
      return names.push(name.first_name.toUpperCase())
    }
  )
    return names;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  var filter = runners.filter(runners => tShirtSize == runners.shirt_size);
  return filter;
}
